...
def update_history_display():
    data = load_win_data()
    total_games = data["even_even"] + data["odd_odd"] + data["draw"]
    history_label.config(
        text=(
            f"Games Played: {total_games}  |  "
            f"Human Wins: {data['even_even']}  |  "
            f"AI Wins: {data['odd_odd']}  |  "
            f"Draws: {data['draw']}"
        )
    )
...
history_frame = tk.Frame(root, bg="#181825", pady=8)
history_frame.pack(fill="x", padx=20, pady=(10, 5), side="bottom")
tk.Label(history_frame, text="📊 Win History", font=("Arial", 11, "bold"), bg="#181825", fg="#cdd6f4").pack()
history_label = tk.Label(history_frame, text="", font=("Arial", 10), bg="#181825", fg="#a6adc8")
history_label.pack()
update_history_display()
